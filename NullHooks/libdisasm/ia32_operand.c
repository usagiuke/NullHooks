...
static void apply_seg( x86_op_t *op, unsigned int prefixes ) {
	...
	switch ( prefixes & PREFIX_REG_MASK ) {
		case PREFIX_CS:
			op->flags = (enum x86_op_flags)(op->flags | op_cs_seg); break;
		case PREFIX_SS:
			op->flags = (enum x86_op_flags)(op->flags | op_ss_seg); break;
		case PREFIX_DS:
			op->flags = (enum x86_op_flags)(op->flags | op_ds_seg); break;
		case PREFIX_ES:
			op->flags = (enum x86_op_flags)(op->flags | op_es_seg); break;
		case PREFIX_FS:
			op->flags = (enum x86_op_flags)(op->flags | op_fs_seg); break;
		case PREFIX_GS:
			op->flags = (enum x86_op_flags)(op->flags | op_gs_seg); break;
	}
	...
	switch (addr_meth) {
		...
		case ADDRMETH_J:	/* Rel offset to add to IP [jmp] */
			/* this fills op->data.near_offset or
			   ...
			   the operand */
			op->flags = (enum x86_op_flags)(op->flags | op_signed);
			if ( op_size == 1 ) {
		...
		case ADDRMETH_F:	/* EFLAGS register */
			op->type = op_register;
			op->flags = (enum x86_op_flags)(op->flags | op_hardcode);
			ia32_handle_register( &op->data.reg, REG_FLAGS_INDEX );
			break;
		case ADDRMETH_X:	/* Memory addressed by DS:SI [string] */
			op->type = op_expression;
			op->flags = (enum x86_op_flags)(op->flags | op_hardcode);
			op->flags = (enum x86_op_flags)(op->flags | op_ds_seg | op_pointer | op_string);
		...
		case ADDRMETH_Y:	/* Memory addressed by ES:DI [string] */
			op->type = op_expression;
			op->flags = (enum x86_op_flags)(op->flags | op_hardcode);
			op->flags = (enum x86_op_flags)(op->flags | op_es_seg | op_pointer | op_string);
			ia32_handle_register( &op->data.expression.base, 
					     REG_DWORD_OFFSET + 7 );
			break;
